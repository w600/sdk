...
/******************************************************************************/
/*
	Init the SEED block cipher context for CBC-EDE mode.
	IV should point to 16 bytes of initialization vector
	Key should point to 16 bytes of data
*/
				   ...
				   unsigned char *key, uint32 keylen)
{
	...
	if (IV == NULL || key == NULL || ctx == NULL || keylen != SSL_SEED_KEY_LEN){
		psTraceCrypto("Bad parameter failure in psSeedInit\n");
		return PS_ARG_FAIL;
	}
/*
	setup cipher
 */
	if ((err = psSeedInitKey(key, keylen, &ctx->seed.key)) != PS_SUCCESS) {
		return err;
	}
/*
	copy IV
 */
	ctx->seed.blocklen = SSL_SEED_IV_LEN;
	for (x = 0; x < ctx->seed.blocklen; x++) {
		ctx->seed.IV[x] = IV[x];
	}
	return PS_SUCCESS;
}
...
int32 psSeedEncrypt(psCipherContext_t *ctx, unsigned char *pt,
					 unsigned char *ct, uint32 len)
{
	...
	if (pt == NULL || ct == NULL || ctx == NULL || (len & 0x7) != 0) {
		psTraceCrypto("Bad parameters to psSeedEncrypt\n");
		return PS_ARG_FAIL;
	}
...
/*
	is blocklen valid?
 */
	if (ctx->seed.blocklen < 0 || (ctx->seed.blocklen > 
			(int32)sizeof(ctx->seed.IV))) {
		psTraceCrypto("Blocklen test failure in psSeedEncrypt\n");
		return PS_LIMIT_FAIL;
	}

	for (i = 0; i < len; i += ctx->seed.blocklen) {
/*
		xor IV against plaintext
 */
		for (x = 0; x < ctx->seed.blocklen; x++) {
			tmp[x] = pt[x] ^ ctx->seed.IV[x];
		}
/*
		encrypt
 */
		psSeedEncryptBlock(tmp, ct, &ctx->seed.key);
 ...
 */
		...
		ct += ctx->seed.blocklen;
		pt += ctx->seed.blocklen;
	}
...
int32 psSeedDecrypt(psCipherContext_t *ctx, unsigned char *ct,
					 unsigned char *pt, uint32 len)
{
	...
	if (pt == NULL || ct == NULL || ctx == NULL || (len & 0x7) != 0) {
		psTraceCrypto("Bad parameters to psSeedDecrypt\n");
		return PS_ARG_FAIL;
	}
 ...
 */
	if (ctx->seed.blocklen < 0 || (ctx->seed.blocklen > 
			(int32)sizeof(ctx->seed.IV))) {
		psTraceCrypto("Blocklen test failure in psSeedDecrypt\n");
		return PS_LIMIT_FAIL;
	}
	for (i = 0; i < len; i += ctx->seed.blocklen) {
/*
 ...
 */
		psSeedDecryptBlock(ct, tmp, &ctx->seed.key);
/*
 ...
 */
			pt[x] =	tmp[x] ^ ctx->seed.IV[x]; 
		}
/*
		replace IV with this current ciphertext
 */
		for (x = 0; x < ctx->seed.blocklen; x++) {
			ctx->seed.IV[x] = tmp2[x];
		}
		ct += ctx->seed.blocklen;
		pt += ctx->seed.blocklen;
	}
 ...
 */
int32 psSeedInitKey(const unsigned char *key, uint32 keylen, psSeedKey_t *skey)
{
	int32	i;
	uint32	tmp, k1, k2, k3, k4;

	if (keylen != 16) {
		psTraceIntCrypto("Invalid key size error in psSeedInitKey: %d\n",
			(int32)keylen);
	...
	for (i = 0; i < 16; i++) {
		skey->K[2*i+0] = G(k1 + k3 - KCi[i]);
		skey->K[2*i+1] = G(k2 - k4 + KCi[i]);
		if (i&1) {
			tmp = k3;
		...
		skey->dK[2*(15-i)+0] = skey->K[2*i+0];
		skey->dK[2*(15-i)+1] = skey->K[2*i+1];
	}
...
*/
void psSeedEncryptBlock(const unsigned char *pt, unsigned char *ct,
						psSeedKey_t *skey)
{
	uint32 P[4];
	LOAD32H(P[0], pt);
...
/**
  ...
  @param skey The key as scheduled 
*/
void psSeedDecryptBlock(const unsigned char *ct, unsigned char *pt,
						psSeedKey_t *skey)
{
	uint32 P[4];
	LOAD32H(P[0], ct);
