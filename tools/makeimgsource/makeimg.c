...
#define IMG_TYPE_NON_ZIP    	(0)
#define IMG_TYPE_ZIP    		(1)
...
int main(int argc,char *argv[])
{
	...
	unsigned int   upd_image_addr = 0;
	unsigned int   run_image_addr = 0;
	...
	switch (argc)
	{
		case 9:
#if _DEBUG_FTR
			printf("original image file for package\n");
#endif
		case 8:
			run_image_addr = strtol(argv[7], NULL, 16);			
#if _DEBUG_FTR
			printf("image run address:0x%x \n", run_image_addr);
#endif
			type = atoi(argv[3]);
			zip_type = atoi(argv[4]);

			upd_image_addr = strtol(argv[6], NULL, 16);		
#if _DEBUG_FTR	
			printf("image download address:0x%x\n", upd_image_addr);
#endif
			break;
		default:
			printf("\nparam cnt error\n");
			printf("参数0:  exe\n");
			printf("参数1:  输入bin文件 ,原始文件或者压缩档文件\n");
			printf("参数2 : 输出文件\n");
			printf("参数3:  输入IMAGE类型,0：user image,1：reserved,2：secboot\n");
			printf("参数4:  输入压缩类型标志，0：非压缩，1：压缩\n");
			printf("参数5:  版本号文件\n");
			printf("参数6： 输入image存放位置\n");
			printf("参数7： 输入image运行位置\n");			
			printf("参数8： 输入原始image文件\n");
			return -1;
	...
	if (strlen(newversion) < 16){
		strcpy(name, newversion);
	}else{
		strncpy(name, newversion,15);
	}
	...
	tbooter.magic_no = IMG_HEAD_MAGIC_NO;
	
	if(IMG_TYPE_NON_ZIP == zip_type)	//non-zip image
	{	
#if _DEBUG_FTR		
		...
		tbooter.run_img_addr = run_image_addr;						
		tbooter.zip_type = zip_type;
		tbooter.upd_img_len = filelen;
		tbooter.upd_img_addr = upd_image_addr;	
		tbooter.upd_checksum = file_crc;
	}
	else  //zip image
	{
		tbooter.upd_img_len = filelen;
		...
		tbooter.run_img_addr = run_image_addr;
    		...
    		printf("argv[8]:%s\n", argv[8]);
#endif    
		//--------Deal with non-zip image's info begin----
		fpsrc = fopen(argv[8],"rb+");
		if(NULL == fpsrc)
		{
		...
		tbooter.run_img_len = filelen;
		tbooter.img_type = type;
		//--------Deal with non-zip image's info end----
	}
	...
	while(!feof(fpbin))
	{
		readlen = fread(buf, 1, ONCE_READ_LEN, fpbin);
#if 0		
		if(readlen < ONCE_READ_LEN && zip_type == IMG_TYPE_ZIP)
		{
			readlen -= 8;
		}
#endif
