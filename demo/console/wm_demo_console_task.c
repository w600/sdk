...
#define   MAX_DEMO_ARG 20	
	...
	for(i = 0; ; i++)
	{
		if (strstr((char *)sys->rx_buf , console_tbl[i].cmd) != NULL)
		{
			/*解析()*/
			...
			if (!pparam_begin && !pparam_end)
			{
				...
				//ret = console_tbl[i].callfn(param[0], param[1], param[2], param[3], param[4]);
				ret = demo_call_fn(console_tbl[i].callfn, param, console_tbl[i].param_cnt);
				if(WM_FAILED == ret)
				{
					printf("\nrun demo failed\n");
				}
		...
		if(strstr(console_tbl[i].cmd, "lastcmd") !=NULL)	//已经到最后一个
		{
			/*wrong cmd parameter,discard this cmd*/
			//demo_console_show_help(NULL);
			return DEMO_CONSOLE_WRONG_CMD;
...
//该任务使用uart0 作为测试控制台，可以输入字符串命令测试其他的demo
void demo_console_task(void *sdata)
{
	...
	tls_uart_rx_callback_register(TLS_UART_0, demo_console_rx);
	...
	for(;;) 
	{
		...
		switch((u32)msg)
		{
			case 1:
				...
				gstConsole.rptr += ret;
				ret = demo_cmd_execute(&gstConsole);	//解析其他demo的测试命令字
				if((DEMO_CONSOLE_CMD == ret) || (DEMO_CONSOLE_WRONG_CMD == ret))	//先进行命令解析，看是否是模拟的命令字
				{
					/*modify*/
					memset(gstConsole.rx_buf , 0, DEMO_CONSOLE_BUF_SIZE);	/*命令传输完成之后清buf*/
					gstConsole.rptr = 0;
				}
