...
/* strndup() is a GNU extension. */
	...
	static char *strndup(const char *__string, size_t __n)
	{
		...
		char *newstr = (char *)tls_mem_alloc(strsize + 1);
		if (newstr == NULL)
			return NULL;
...
UpnpString *UpnpString_new()
{
	...
	struct SUpnpString *p = tls_mem_calloc((size_t)1, sizeof (struct SUpnpString));
	if (p == NULL) {
		goto error_handler1;
	}
#if 0
	...
	p->m_string = tls_mem_calloc((size_t)1, (size_t)1);
	if (p->m_string == NULL) {
		goto error_handler2;
	}
...
{
	struct SUpnpString *q = tls_mem_calloc((size_t)1, sizeof (struct SUpnpString));
	if (q == NULL) {
		goto error_handler1;
	}
...
int UpnpString_set_String(UpnpString *p, const char *s)
{
	...
	tls_mem_free(((struct SUpnpString *)p)->m_string);
