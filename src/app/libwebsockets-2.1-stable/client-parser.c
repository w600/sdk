...
spill:
		...
		case LWSWSOPC_CLOSE:
			...
			lws_write(wsi, (unsigned char *)&wsi->u.ws.rx_ubuf[LWS_PRE],
				  wsi->u.ws.rx_ubuf_head, (enum lws_write_protocol)LWS_WRITE_CLOSE);
...
utf8_fail:			lwsl_info("utf8 error\n");
		...
		if (wsi->state == LWSS_RETURNED_CLOSE_ALREADY ||
			...
			goto already_done;
lwsl_debug("%s: callback %p, callback_action %d, token %.*s, len %d\n", __func__, wsi->protocol->callback, callback_action, eff_buf.token_len, eff_buf.token, eff_buf.token_len);
