...
lws_create_context(struct lws_context_creation_info *info)
{
	...
	context->time_up = tls_os_get_time()/HZ;
#ifndef LWS_NO_DAEMONIZE
	if (pid_daemon) {
	...
	for (n = 0; n < context->count_threads; n++) {
		...
		if (!context->pt[n].ah_pool)
		{
			...
			lwsl_err("ah_pool is null, malloc size %d failed, max_http_header_pool %d\n", context->pt[n].ah_pool, sizeof(struct allocated_headers) *
					      context->max_http_header_pool, context->max_http_header_pool);
			goto bail;
		}
