	...
	while (m--) {
		for (n = 0; n < pt->fds_count; n++) {
			...
			if (wsi->protocol == protocol)
				protocol->callback(wsi, (enum lws_callback_reasons)reason, wsi->user_space,
						   NULL, 0);
		}
		pt++;
	}
...
lws_callback_vhost_protocols(struct lws *wsi, int reason, void *in, int len)
{
	...
	for (n = 0; n < wsi->vhost->count_protocols; n++)
		if (wsi->vhost->protocols[n].callback(wsi, (enum lws_callback_reasons)reason, NULL, in, len))
			return 1;
...
lwsl_timestamp(int level, char *p, int len)
{
	...
	time_t o_now = tls_os_get_time()/HZ;
