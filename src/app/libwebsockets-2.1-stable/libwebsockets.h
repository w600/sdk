...
struct lws_plat_file_ops {
	lws_filefd_type (*open1)(struct lws *wsi, const char *filename,
	...
	int (*close1)(struct lws *wsi, lws_filefd_type fd);
	/**< close file */
	unsigned long (*seek_cur)(struct lws *wsi, lws_filefd_type fd,
				  long offset_from_cur_pos);
	/**< seek from current position */
	int (*read1)(struct lws *wsi, lws_filefd_type fd, unsigned long *amount,
		    unsigned char *buf, unsigned long len);
	/**< Read from file, on exit *amount is set to amount actually read */
	int (*write1)(struct lws *wsi, lws_filefd_type fd, unsigned long *amount,
		     unsigned char *buf, unsigned long len);
...
lws_plat_file_open(struct lws *wsi, const char *filename,
		   unsigned long *filelen, int flags)
{
	return lws_get_fops(lws_get_context(wsi))->open1(wsi, filename,
						    filelen, flags);
}
...
{
	return lws_get_fops(lws_get_context(wsi))->close1(wsi, fd);
}
...
lws_plat_file_read(struct lws *wsi, lws_filefd_type fd, unsigned long *amount,
		   unsigned char *buf, unsigned long len)
{
	return lws_get_fops(lws_get_context(wsi))->read1(wsi, fd, amount, buf,
							len);
}
/**
...
lws_plat_file_write(struct lws *wsi, lws_filefd_type fd, unsigned long *amount,
		    unsigned char *buf, unsigned long len)
{
	return lws_get_fops(lws_get_context(wsi))->write1(wsi, fd, amount, buf,
							 len);
}
//@}
