...
lws_ssl_capable_read(struct lws *wsi, unsigned char *buf, int len)
{
...
#if defined(LWS_USE_POLARSSL)
	n = HTTPWrapperSSLRecv(wsi->ssl, wsi->sock, buf, len, 0);
	if(n == SOCKET_SSL_MORE_DATA)
		n = len;
#else
#if defined(LWS_USE_MBEDTLS)
#else
	...
	if (!wsi->ssl)
		goto bail;
#if defined(LWS_USE_POLARSSL)
	if (HTTPWrapperSSLRecvPending(wsi->ssl) <= 0)
		goto bail;
#else
#if defined(LWS_USE_MBEDTLS)
#else
...
lws_ssl_pending(struct lws *wsi)
{
	if (!wsi->ssl)
		return 0;
#if defined(LWS_USE_POLARSSL)
	return HTTPWrapperSSLRecvPending(wsi->ssl) > 0;
#else
#if defined(LWS_USE_MBEDTLS)
...
#if defined(LWS_USE_POLARSSL)
	n = HTTPWrapperSSLSend(wsi->ssl, wsi->sock, buf, len, 0);
#else
#if defined(LWS_USE_MBEDTLS)
#else
...
lws_ssl_close(struct lws *wsi)
{
	...
	HTTPWrapperSSLClose(wsi->ssl, wsi->sock);
#else
#if defined(LWS_USE_MBEDTLS)
#else
...
lws_server_socket_service_ssl(struct lws *wsi, lws_sockfd_type accept_fd)
{
...
#if defined(LWS_USE_POLARSSL)
	{
		...
		ssl_set_session(wsi->ssl, 1, 600, ssn);
