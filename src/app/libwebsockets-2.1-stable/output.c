			  ...
			  enum lws_write_protocol wp)
{
	...
	if (wsi->state == LWSS_ESTABLISHED && wsi->u.ws.tx_draining_ext) {
		...
		wp = (enum lws_write_protocol)((wsi->u.ws.tx_draining_stashed_wp & 0xc0) |
	...
	default:
		...
		if (eff_buf.token_len && wsi->u.ws.stashed_write_pending) {
			...
			wp = (enum lws_write_protocol)((wp &0xc0) | (int)wsi->u.ws.stashed_write_type);
		}
	}
...
#if LWS_POSIX
	n = send(wsi->sock, (char *)buf, len, 0);
