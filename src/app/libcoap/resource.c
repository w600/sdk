...
#ifdef WITH_LWIP
  r = (coap_resource_t *)tls_mem_alloc(sizeof(coap_resource_t));
#endif
#ifndef WITH_LWIP
...
#ifdef WITH_LWIP
  attr = (coap_attr_t *)tls_mem_alloc(sizeof(coap_attr_t));
#endif
#ifndef WITH_LWIP
...
coap_hash_request_uri(const coap_pdu_t *request, coap_key_t key) {
  ...
  while ((option = coap_option_next(&opt_iter))!=0)
