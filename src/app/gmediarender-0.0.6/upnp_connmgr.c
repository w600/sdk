...
static void register_mime_type_internal(const char *mime_type)
{
	...
	UpnpPrintf( UPNP_INFO, API, __FILE__, __LINE__,
		"Registering support for '%s'\n", mime_type);
	entry = tls_mem_alloc(sizeof(struct mime_type));
...
int connmgr_init(void)
{
	...
	for (entry = supported_types; entry; entry = entry->next) {
		...
		offset = p - buf;
		buf = tls_mem_realloc(buf, bufsize);
		if (buf == NULL) {
...
void connmgr_destroy(void)
{
	char* buf = connmgr_values[CONNMGR_VAR_SINK_PROTO_INFO] ;
	if(buf)
	{
		tls_mem_free(buf);
		connmgr_values[CONNMGR_VAR_SINK_PROTO_INFO] = NULL;
	}
}
