...
static int ssl_encrypt_buf( ssl_context *ssl )
{
    ...
    else
    {
        ...
        IGNORE_PARAMETER(enc_msg);
        IGNORE_PARAMETER(enc_msglen);
...
static int ssl_decrypt_buf( ssl_context *ssl )
{
    ...
    else
    {
        ...
        unsigned char *dec_msg_result = NULL;
        size_t dec_msglen = 0;

        IGNORE_PARAMETER(dec_msg_result);
...
int ssl_parse_certificate( ssl_context *ssl )
{
    ...
    if( ( ssl->peer_cert = (x509_cert *) polarssl_malloc(
