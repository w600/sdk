                         ...
                         u8 *ehdr, u16 eth_len)
{
    ...
    if (0 == compare_ether_addr(bssid, mac2))
    {
        if ((ip_hdr->dest.addr == net_if->next->ip_addr.addr) ||
            (ip_hdr->dest.addr == net_if->ip_addr.addr))
        {
...
redo:
    ...
    else
    {
        ...
        if (NULL != napt)
        {
            ...
            {
                ip_hdr->src.addr = net_if->next->ip_addr.addr;
            }
...
static int alg_gre_proc(const u8 *bssid, struct ip_hdr *ip_hdr, u8 *ehdr, u16 eth_len)
{
    ...
    {
        ip_hdr->dest.addr = (gre_info.src_ip << 24) | (net_if->next->ip_addr.addr & 0x00ffffff);
        ip_hdr->_chksum = 0;
        ip_hdr->_chksum = alg_iphdr_chksum((u16 *)ip_hdr, iphdr_len);
